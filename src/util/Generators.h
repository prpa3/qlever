...
class CachingWrapper
 ...
 public:
  ...
  CachingWrapper(const CachingWrapper&) = delete;
  CachingWrapper& operator=(const CachingWrapper&) = delete;
  CachingWrapper(CachingWrapper&&) = default;
  CachingWrapper& operator=(CachingWrapper&&) = default;

  ~CachingWrapper() { callOnFullyCachedIfNeeded(); }
...
private:
  void callOnFullyCachedIfNeeded() {
    if (shouldBeAggregated_ && aggregatedData_.has_value()) {
      shouldBeAggregated_ = false; // Prevent multiple calls
